...
# ── Paths ──────────────────────────────────────────────────────────────────────
# Project root (one level above `src`) so data lives at the repository root
...
def get_db_connection(db_path: Path = DB_PATH) -> sqlite3.Connection:
    """Open a SQLite connection safe for use in a worker thread."""
...
def download_ticker(
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            log.warning(f"{ticker}: download error on attempt {attempt} — {e}")
        ...
        if raw.empty:
            if attempt < max_attempts:
                log.warning(
                    f"{ticker}: no data returned on attempt {attempt}; retrying in "
                    f"{retry_delay * attempt:.1f}s"
                )
                time.sleep(retry_delay * attempt)
                continue
